...
import org.healthchecktool.http.invoke.withoutcert.jmxHttpConnectWithoutCert;
import org.healthchecktool.util.logger.logger;

import sun.misc.BASE64Encoder;

public class jmxHttpInvokeImpl implements jmxHttpInvoke {
    private 	String 				host;
    private 	int 				port;
    private 	String 				user;
    private 	String 				pass;
    ...
    //############################CONSTRUCTORS#################################################################
    public jmxHttpInvokeImpl(String host,int port,String user,String pass) {
        setJmxInvokeCredentials(host,port,user,pass);
    }
    public jmxHttpInvokeImpl(String host,int port,String user,String pass,logger log) {
        setJmxInvokeCredentials(host,port,user,pass);
        setLog(log);
    }
    //#######################################INTERFACE########################################################
    ...
    private void writeData(HttpURLConnection connection)throws IOException {
        String data = params;//URLEncoder.encode(params, "UTF-8");
